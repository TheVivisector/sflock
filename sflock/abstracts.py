...
class Unpacker(object):
    ...
    def zipjail(self, filepath, dirpath, *args):
        ...
        p = subprocess.Popen(
            (zipjail, filepath, dirpath, "--", self.exe) + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
class File(object):
    ...
    def to_dict(self):
        return {
            ...
            "finger": {"magic": self.magic, "mime": self.mime, "mime_human": self.mime_human, "magic_human": self.magic_human,},
    ...
    def astree(self, finger=True, sanitize=False):
        ...
        def findentry(entry, name):
            ...
            entry.append(
                {"type": "directory", "filename": name, "children": [], "preview": True,}
            )
            return entry[-1]
