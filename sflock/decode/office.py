...
class Office(Decoder):
    ...
    def init_secret_key(self):
        ...
        if self.password:
            ...
            block_encrypted_key = bytearray([0x14, 0x6E, 0x0B, 0xE7, 0xAB, 0xAC, 0xD0, 0xD6,])

            # AES decrypt the encrypted* values with their pre-defined block
            # keys and salt in order to get secret key.
            aes = Cipher(
                algorithms.AES(self.gen_encryption_key(block_verifier_input)), modes.CBC(self.ei.password_salt), backend=default_backend()
            ).decryptor()
            self.verifier_hash_input = aes.update(self.ei.verifier_hash_input) + aes.finalize()

            aes = Cipher(
                algorithms.AES(self.gen_encryption_key(block_verifier_value)), modes.CBC(self.ei.password_salt), backend=default_backend()
            ).decryptor()
            self.verifier_hash_value = aes.update(self.ei.verifier_hash_value) + aes.finalize()

            aes = Cipher(
                algorithms.AES(self.gen_encryption_key(block_encrypted_key)), modes.CBC(self.ei.password_salt), backend=default_backend()
            ).decryptor()
